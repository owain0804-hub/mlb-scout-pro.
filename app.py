...
if not st.session_state.auth and not st.session_state.is_guest:
    ...
    with t2:
        ...
        if st.button("Create Account"):
            if nu and np:
                users_db[nu] = {"pw": hash_pw(np), "weights": [30, 40, 15, 15], "display_model": 1}
                save_users(users_db); st.success("Created! Go to Login.")
    st.stop()

# --- SIDEBAR (WEIGHTS UPDATED) ---
user_data = users_db.get(st.session_state.username, {"weights": [30, 40, 15, 15], "display_model": 1})
...
def analyze_game(gid, g_info, year, weights):
    ...
    # Impact calculations without Bullpen
    imp = {
    ...
    p_final = 0.5 + sum(imp.values()) + 0.02 # Includes +2% Home Edge
...
for g in sched:
    ...
    if st.button("Analyze", key=g['game_id'], use_container_width=True):
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f'<div class="pitcher-header">{data["away"]["p"]} ({data["away"]["era"]})</div>', unsafe_allow_html=True)
            st.dataframe(pd.DataFrame(data['away']['lineup']), hide_index=True)
        with c2:
            st.markdown(f'<div class="pitcher-header">{data["home"]["p"]} ({data["home"]["era"]})</div>', unsafe_allow_html=True)
