...
def get_detailed_data(game_id, game_info):
    try:
        ...
        def get_strength_metrics(side, team_name):
            ...
            # CORE WEIGHTING (Unchanged as requested)
            adj_wpct = (t_info['wpct'] * 0.45) + (l_avg * 1.0) + (l_slg * 0.8) + (bp_score * 0.12) + ((4.1/pitching_skill) * 0.15)
            
            # THE "SENSITIVITY" FIX:
            # We apply an exponent to the raw score to amplify differences.
            # This pushes the model away from 50/50 and toward more decisive predictions.
            spread_factor = 1.75 
        ...
        # Log-5 Probability Formula (Now receives amplified inputs)
        pa, pb = a['wpct'], h['wpct']
...
# UI Rendering (Exact same as before)
