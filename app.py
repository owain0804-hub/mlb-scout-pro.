...
def get_detailed_data(game_id, game_info):
    try:
        ...
        def get_strength_metrics(side, team_name):
            ...
            p_era = float(sp_data.get('era', 4.10))
            p_fip = float(sp_data.get('fip', 4.10))
            ...
            # WEIGHTS: Standings(42%), AVG(11%), SLG(14%), ERA(15%), FIP(6%), Bullpen(12%)
            adj_wpct = (t_info['wpct'] * 0.42) + (l_avg * 0.44) + (l_slg * 0.35) + \
                       ((4.1/p_era) * 0.15) + ((4.1/p_fip) * 0.06) + (bp_war * 0.12)
            
            spread_factor = 1.01 
            amplified_wpct = (adj_wpct**spread_factor) / ((adj_wpct**spread_factor) + ((1-adj_wpct)**spread_factor))
            
            return {"name": game_info.get(f'{side}_probable_pitcher', "TBD"), "stats": sp_data, 
                    "wpct": amplified_wpct, "div_id": t_info['div_id'], "lineup": lineup_names, 
...
with col_l:
    st.image("https://www.mlbstatic.com/team-logos/league-laundry/mlb.svg", width=80)
with col_r:
        ...
        if st.button("Analyze", key=str(g['game_id'])):
            ...
            if data:
                ...
                st.markdown(f"""<div style="display:flex; justify-content:space-around; background:#111; padding:15px; border-radius:10px; border:1px solid #333; color:white;">
                    ...
                    <div style="text-align:center;"><img src="https://www.mlbstatic.com/team-logos/league-laundry/mlb.svg" width="40"><br>VS</div>
